...
class PlayState extends MusicBeatState
{
	...
	var funnyFloatyBoys:Array<String> = ['ben-3d'];
	...
	var shark:FlxSprite;
	var bgshark:FlxSprite;
	var bg2:FlxSprite = new FlxSprite(-140, -240).loadGraphic(Paths.image('duolingoBg2'));
	var Apprentice3DBG:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave/3dbg'));
	...
	var pires:BGSprite;
	var ben:BGSprite;
	var estevam:BGSprite;
	var pi3D:BGSprite;
	var estevam3D:BGSprite;
	var heyTimer:Float;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'woody': //Week Woody
				var bg:BGSprite = new BGSprite('Week-Woody/background', -600, -100, 0.9, 0.9);
				add(bg);

				pi = new BGSprite('Week-Woody/pipoca_dance', -300, 500, 1, 1, ['Pipoca but bruh']);
				add(pi);

				estevam = new BGSprite('Week-Woody/estevam_dance', 1160, 500, 1, 1, ['estevam66 dance']);
				add(estevam);
			
			case 'restaurant': //Restaurant Week
				var bg:BGSprite = new BGSprite('Restaurant-Week/background', -350, -281, 0.9, 0.9);
				add(bg);

				pires = new BGSprite('Restaurant-Week/pipoca', 1200, 250, 1, 1, ['Pipoca Fundo Idle']);
				add(pires);

				ben = new BGSprite('Restaurant-Week/ben', 760, 170, 1, 1, ['Ben Fundo Idle']);
				add(ben);
			
			case 'restaurant-noben': //Restaurant Week
				var bg:BGSprite = new BGSprite('Restaurant-Week/background', -350, -281, 0.9, 0.9);
				add(bg);
			...
			case 'epic': //Flushed
				var bg:BGSprite = new BGSprite('Week-Woody/Epic_BGames', -600, -100, 0.9, 0.9);
				add(bg);
			
			case 'woody3d': //Apprentice
				var bg:BGSprite = new BGSprite('Week-Woody/background', -600, -100, 0.9, 0.9);
				add(bg);
				...
				Apprentice3DBG.loadGraphic(Paths.image('dave/3dbg'));
				Apprentice3DBG.antialiasing = true;
				Apprentice3DBG.scrollFactor.set(0.6, 0.6);
				Apprentice3DBG.active = true;
				Apprentice3DBG.visible = false;
				add(Apprentice3DBG);

				pi3D = new BGSprite('dave/pipoca_3d', -100, 100, 1, 1);
				pi3D.active = true;
				pi3D.visible = false;
				add(pi3D);
			    
				estevam3D = new BGSprite('dave/estevam_3d', 1100, 100, 1, 1);
				estevam3D.active = true;
				estevam3D.visible = false;
				add(estevam3D);
				...
				Apprentice3DBG.shader = testshader.shader;
				curbg = Apprentice3DBG;
			
			case 'void': //Lesson
				var bg:BGSprite = new BGSprite('extra/duolingoBg', -100, -110, 0.9, 0.9);
				add(bg);

				bg2.loadGraphic(Paths.image('extra/duolingoBg2'));
			...
			case 'woody-night': //Rapping Night
				var bg:BGSprite = new BGSprite('Week-Woody/background_night', -600, -100, 0.9, 0.9);
				add(bg);
				...
				pi.color = 0xFF878787;
				estevam.color = 0xFF878787;
				bg.color = 0xFF878787;
			
			case 'house_woody': //Talking Two
				var bg:BGSprite = new BGSprite('Week-Woody/house_background', -100, -270, 0.9, 0.9);
				add(bg);

				linha = new BGSprite('Week-Woody/the linha', 640, -290, 1, 1, ['negocio do fundo']);
				add(linha);
			
			case 'woody-afternoon': //Roots
				var bg:BGSprite = new BGSprite('Week-Woody/background_afternoon', -600, -100, 0.9, 0.9);
				add(bg);
			...
			case 'xbox': //XBOX
				var bg:BGSprite = new BGSprite('extra/XBOXBG', -600, -100, 0.9, 0.9);
				add(bg);

			case 'sharkbite':
				var shark = Paths.getSparrowAtlas('extra/shark_escape_bg');

				bgshark = new FlxSprite(-1330, -1080);
				bgshark.frames = shark;
				bgshark.animation.addByPrefix('idle', 'BG');
				bgshark.animation.play('idle');
				bgshark.antialiasing = true;
				add(bgshark);
			
			case 'hospital': //Fimose and Wheels
				var bg:BGSprite = new BGSprite('extra/hospital', -600, -200, 0.9, 0.9);
				add(bg);
			
			case 'hospital-poopers': //Poopers
				var bg:BGSprite = new BGSprite('extra/hospital', -600, -200, 0.9, 0.9);
				add(bg);

			case 'woody-old': //Older
				var bg:BGSprite = new BGSprite('Woody Street.flp/background', -111, -60, 0.9, 0.9);
				add(bg);

				dance = new BGSprite('Woody Street.flp/dance', -10, 260, 1, 1, ['pipoca e estevam dance']);
				add(dance);
		}
		...
		timeBar = new FlxBar(timeBarBG.x + 100000000000000, timeBarBG.y + 4, LEFT_TO_RIGHT, Std.int(timeBarBG.width - 8), Std.int(timeBarBG.height - 8), this,
			'songPercent', 0, 1);//bro tem que ser igual ao golden apple shh nem reclama /j
	...
	if(funnyFloatyBoys.contains(boyfriend.curCharacter.toLowerCase()) && canFloat)
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			switch (curStep)
			{
				case 671:
				    FlxG.camera.flash(FlxColor.WHITE, 1);
                    Apprentice3DBG.visible = true;
					pi3D.visible = true;
					estevam3D.visible = true;
				case 1456:
				    FlxG.camera.flash(FlxColor.WHITE, 1);
                    Apprentice3DBG.visible = false;
					pi3D.visible = false;
		...
		switch (curStage)
		{
			case 'woody-night': // Dark character thing
                {
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				'week2_nomiss', 'week7_nomiss', 'ur_bad',
  ...
  if (curBeat % gfSpeed == 0) {
   ...
   iconP2.updateHitbox();
		...
		switch (curStage)
		{
			...
			case 'restaurant-noben':

				pires.dance(true);
			
			case 'woody-afternoon':
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'week7_nomiss' | 'week8_nomiss':
